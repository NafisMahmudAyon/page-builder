    ...
    <>
      {block.type === "heading" && (
        <Heading
          ...
          }} blockData={block} className={cn(selected && block.id === selected.id && "border border-border/30 border-dashed", responsive === 'lg' ? block.options?.block?.className : findBlockById(responsiveBlock, block.id)?.options?.block?.className)}>
      ...
      {block.type === "image" && (
        ...
        } className={`${selected && block.id === selected.id && "border border-border/30 border-dashed"}`} />
      )}
      {block.type === "text" && (
        <Text onClick={() => {
          setSelected(block)
